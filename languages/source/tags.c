...
// There are NS symbols and NL letters, making NT tag characters.
enum { NS = 32, NL = 26, NT = NS + NL };
static char tagChars[] =
    "!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~ABCDEFGHIJKLMNOPQRSTUVWXYZ";

// An operator says what happens when bracket or delimiter L meets another R.
// During forward matching, L is on the stack and R is next in the input. Then
// EQ means L and R match, so L is popped. GT means L and R mismatch, with R
// having lower priority and being flagged as an error token. LT means L and R
...
// backward matching, R is on the stack and L is next in the input, and the
