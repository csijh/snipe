...
// that it can be saved at any moment by autosave. That means, between
// operations, the text doesn't contain invalid UTF-8 sequences, control
// characters other than newlines, trailing spaces at the ends of lines,
// trailing blank lines, or a missing final newline. The bytes are held in a gap
// buffer. For n bytes, there are n+1 positions in the text, running from 0 (at
// the start) to n (after the final newline).
...
// TODO load, save, clean (insert, delete, repair) set line.
// repair: desired indent, semicolon precedes
