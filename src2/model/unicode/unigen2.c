...
enum { MAX = 1114112 };
static short fullTable[MAX];
static byte indexTable[MAX/256];
static short dataTable[MAX];
...
static void fillCatTable(char *path, short *fullTable) {
...
static void fillGraphTable(char *path, short *fullTable) {
    char *text = readFile(path);
    for (int i = 0; i < MAX; i++) fullTable[i] = fullTable[i] | (OR << 5);
    ...
    for (int p = 0; p < n; p++) {
        ...
        for (int i = start; i <= end; i++) {
            fullTable[i] = fullTable[i] | (value << 5);
        }
    }
    free(text);
}
...
static void overrideGraphTable(char *path, short *fullTable) {
    char *text = readFile(path);
    int start, end, n = strlen(text);
    char *code, *endcode, *property;
    for (int p = 0; p < n; p++) {
        ...
        for (int i = start; i <= end; i++) {
            ...
            fullTable[i] = (fullTable[i]&0x1F) | (EP << 5);
        }
    }
    free(text);
}
...
static int pack(short *fullTable, byte *indexTable, short *dataTable) {
    int indexSize = 0, dataSize = 0;
    for (int b = 0; b < MAX; b = b + 256) {
        int address = -1;
        for (int ob = 0; ob < dataSize; ob = ob + 256) {
            if (memcmp(&fullTable[b], &dataTable[ob], 512) == 0) {
                address = ob / 256;
                break;
            }
        }
